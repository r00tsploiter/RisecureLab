...
Function AdditionalChecks { 
    ...
    $LowRAM = $True
    If ($RamInGb -gt 6) {
        Write-Host 'Server RAM check - 4 to 6 Gb - ' -NoNewline
        Write-Host 'Failed' -ForegroundColor Red -NoNewline
        Write-Host ' - [Not Optimal] - over 4 GB' -ForegroundColor Yellow
        $LowRAM = $False
    }
    If ($LowRAM) {
        If ($RaminGb -gt 4) {
            ...
            Write-Host ' - [Optimal] - 4 to 6 Gb - RAM'
        } Else {
            Write-Host 'Server RAM check - 4 to 6 Gb - ' -NoNewline
            Write-Host 'Failed' -ForegroundColor red -NoNewline
            Write-Host ' - [Not Optimal] - Under 4 GB' -ForegroundColor Yellow
        }
    }
...
Function CPlusPlus {
    # Ask the user to input the download directory
    $DownloadFolder = Read-Host "Enter the directory for file download (e.g., C:\Downloads)"
    ...
    If ($val.Version -ge '184610406') {
        Write-Host 'Microsoft Visual C++ 2012 x64 Runtime - 11.0.61030 is already installed.' -ForegroundColor Green
    } Else {
        ...
        $val = Get-ItemProperty -Path "HKLM:\SOFTWARE\Classes\Installer\Products\C3AEB2FCAE628F23AAB933F1E743AB79" -ErrorAction Silentlycontinue
        If ($val.Version -ge '184610406') {
            Write-Host 'Microsoft Visual C++ 2012 x64 Runtime - 11.0.61030 is now installed.' -ForegroundColor Green
        } Else {
            Write-Host 'Microsoft Visual C++ 2012 x64 Runtime was not detected! ' -ForegroundColor Yellow -NoNewline
            Write-Host 'A reboot may be needed.' -ForegroundColor White
        }
    }
}
