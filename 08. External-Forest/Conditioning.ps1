...
Switch ($Progress) {
    ...
    {$_ -le 6} {
        ...
        # Variables for the trust relationship
        $RisecureLab = "Risecure.lab"  # Replace with your first forest's domain name
        $CompanyX = "CompanyX.corp"  # Replace with your second forest's domain name
        $TrustPassword = "YourTrustPassword"  # Set a secure password for the trust relationship

        # Create a transitive trust from Forest A to Forest B
        New-ADTrust -Name $CompanyX -SourceForest $RisecureLab -TargetForest $CompanyX -Direction Bidirectional -TrustType Forest -TrustPassword (ConvertTo-SecureString $TrustPassword -AsPlainText -Force) -AuthenticationType "Both"

        # Create a transitive trust from Forest B to Forest A
        New-ADTrust -Name $RisecureLab -SourceForest $CompanyX -TargetForest $RisecureLab -Direction Bidirectional -TrustType Forest -TrustPassword (ConvertTo-SecureString $TrustPassword -AsPlainText -Force) -AuthenticationType "Both"

        # Verify the trust relationships
        Get-ADTrust -Filter * | Where-Object {$_.SourceForest -eq $RisecureLab -or $_.SourceForest -eq $CompanyX}

        Write-Output "Transitive Trust Set in Risecure.lab and CompanyX.corp!!"
